...
const MainInfo = ({ product, otherInfo }) => {
  ...
  const info = [
    {
      icon: <BadgeCheck />,
      key: t("guarantee.title"),
      text: t("guarantee.value"),
    },
    {
      icon: <Store />,
      key: t("stock.title"),
      text: isInStock ? t("stock.available") : t("stock.outOfStock"),
    },
    { icon: <Truck />, key: t("delivery.title"), text: t("delivery.value") },
  ];

  const [selectedColor, setSelectedColor] = useState(
    currentVariant?.variant_options?.find((o) => o.key === "Color")?.value ||
      null,
  );
  const [selectedStorage, setSelectedStorage] = useState(
    currentVariant?.variant_options?.find((o) => o.key === "Storage")?.value ||
      null,
  );
  ...
  const storageOptions = variants
    ...
    .map((v) => ({
      id: v.id,
      storage:
        v.variant_options?.find((o) => o.key === "Storage")?.value || "N/A",
  ...
  const ramOptions = variants
    .filter((v) => {
      const color =
        v.variant_options?.find((o) => o.key === "Color")?.value || "N/A";
      const storage =
        v.variant_options?.find((o) => o.key === "Storage")?.value || null;
      return (
        color === selectedColor &&
        storage === selectedStorage &&
        v.variant_options?.find((o) => o.key === "RAM")
      );
    })
    .map((v) => ({
      id: v.id,
  ...
  const currentColor = currentVariant?.variant_options?.find(
    (o) => o.key === "Color",
  )?.value;
  const currentStorage = currentVariant?.variant_options?.find(
    (o) => o.key === "Storage",
  )?.value;
  const currentRam = currentVariant?.variant_options?.find(
    (o) => o.key === "RAM",
  )?.value;
  ...
  return (
    <div className="h-full w-full">
      ...
      <ProductPrice
        ...
        discountPercent={
          currentVariant?.discount_percent ?? currentVariant?.discountPercent
        }
            ...
            {specs.map(
              (spec, index) =>
                productSpecs.some((s) => s.key === spec) && (
                  <li
                    className="w-full rounded p-2 flex flex-col   bg-surface"
                    key={index}
                  >
                    <span className="font-semibold text-text">{spec}</span>
                    <span className="text-text">
                      {productSpecs.find((s) => s.key === spec)?.value}
                    </span>
                  </li>
                ),
            )}
      ...
      <div className="grid grid-cols-2 gap-4 mt-6">
        <div
          className={`border-2 border-border rounded flex center hover:border-danger transition ${otherInfo?.isFavorite ? "border-danger" : null}`}
        >
          <AddToWishListCom
